...
public class JetGameView extends SurfaceView implements SurfaceHolder.Callback, World.StateListener {
    ...
    private static final String SPRITE_FILE = "sprites.jpg";

    private GameThread gameThread; // controls the game loop
    private ShipWorld world;
    ...
    MediaPlayer player = MediaPlayer.create(getContext(), R.raw.main_theme_overworld);;


    private enum CurrentStage {StageOne, StageTwo};
    private CurrentStage current = CurrentStage.StageOne;
                ...
                new DialogFragment() {
                    ...
                    public Dialog onCreateDialog(Bundle bundle) {
                        ...
                        builder.setMessage(getResources().getString(messageId) + "\n" + getResources().getString(
                                ...
                                world.projectilesFired,//world.shotsFired,
                                world.enemiesKilled,//world.kills,
                                world.enemies.size(),//world.remaining,
                                world.points,//world.score,
    ...
    public void onGameOver(boolean lost) {
        ...
        if(lost) {
            soundManager.playSound(SoundManager.JET_HIT);
            current = CurrentStage.StageOne;
            showGameOverDialog(R.string.lose); // show the losing dialog
        }
        else {
            if(current == CurrentStage.StageOne)
                current = CurrentStage.StageTwo;
            else
                current = CurrentStage.StageOne;
